...
For a fixed image resolution and a fixed training dataset, a useful first-order view is
\begin{equation}
T_{\mathrm{train}}
\approx
\bar{N}_{\mathrm{gs}}
...
N_{\mathrm{iter}},
\end{equation}
where $\bar{N}_{\mathrm{gs}}$ is the average number of Gaussians during training, $c_{\mathrm{opt}}$ is the average cost of one step per Gaussian under the current rasterization and optimizer path, and $N_{\mathrm{iter}}$ is the number of optimization steps. The terms are not independent. Increasing the Gaussian count can also increase tile-instance count and memory pressure. Reducing the iteration count can change which densification and pruning events happen. Fusing the optimizer can change the cost profile even when the Gaussian count is unchanged.
