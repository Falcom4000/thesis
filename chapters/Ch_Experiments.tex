...
Figures~\ref{fig:qualitative-garden} and~\ref{fig:qualitative-room} compare
aligned held-out views from the same scene cameras. The panels are resized to a
common display width because the saved RapidGS main run uses a different output
resolution from the two 30k baselines. No sharpening, denoising, or color
correction is applied. The red rectangle marks the region enlarged in the
second row.
...
shows floor seams and foreground occlusion boundaries. RapidGS remains in the
same visual-quality range as 3DGS and the local FastGS reference while using a
shorter training schedule and fewer final Gaussians.}
