...
These measurements establish where fused update time is accounted for, but they are not a matched PyTorch-versus-fused microbenchmark. A strict fusion speedup table must hold the Gaussian set, visibility lists, image gradient, and optimizer state fixed and time the two update paths on the same GPU. Until that matched experiment is available, the thesis uses end-to-end backend comparisons in Chapter~\ref{chap:experiments} and does not attribute the full training-time difference to fusion alone.
