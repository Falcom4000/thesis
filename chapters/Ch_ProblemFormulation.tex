...
\end{equation}
which supports rendering of both training and novel views through standard 3DGS rasterization.
...
The $1/2$ offset follows the common 3DGS coefficient convention. The maintained fused implementation computes this raw SH value during preprocessing and applies a component-wise non-negative clamp when the blend kernel loads the color. Backward propagation uses a zero derivative for any channel whose raw SH value is negative. The training render has no final upper clamp at one, whereas the inference output is clamped to $[0,1]$ after rasterization. This is an implementation-level numerical convention rather than a new color representation.
